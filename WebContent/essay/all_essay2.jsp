...
<a href="javascript:;" id="btn" title="回到顶部"></a>
		...
		<li class="dropdown navbar-right">
			...
			style="color: forestgreen; font-size: 18px"><%=sname%> <b
				...
				<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1">
					<li>
						<a href="../myspace/my_info.jsp" tabindex="-1" >我的空间</a>
					</li>
					<li>
						<a href="<%=request.getContextPath()%>/index.jsp" tabindex="-1" >注销</a>
					</li>
				</ul>
			</li>
			<img src="../image/头像.jpg" class="img-circle navbar-right" height="40" width="40" id="头像"/>&nbsp;&nbsp;&nbsp;
		...
		<ul id="ul02">
			<li>
				<a href="../essay/write_essay.jsp" class="LeftTag">
					写文章
				</a>
			</li>
			<li>
				<a href="../question/ask_question.jsp" class="LeftTag">
					提&nbsp;&nbsp;&nbsp;&nbsp;问
				</a>
			</li>
			<li>
				<a href="../answer/write_answer.jsp" class="LeftTag">
					写回答
				</a>
			</li>
		</ul>
<div id="div01">
		<p id="p01" style="font-size:200%">所有文章</p>
		<br><br>
			 ...
			 <%
				...
				//每页显示的成员列表
				List<Article> list = art.getPageIntroductionList(pageIndex,pageSize,null);
				//request.setAttribute("introList", introList);
				//request.setAttribute("pageIndex", pageIndex);
				Iterator<Article> iter = list.iterator();
   int i = 0;
   while(iter.hasNext()){
	   Article arts = iter.next();
	   String str = null;
       if(arts.getAtext().length() > 100)
       	str = arts.getAtext().substring(0,100);
       else
       	str = arts.getAtext();
%>
<p id="p02">
	<a href="essay_content.jsp?name=<%=arts.getAno()%>"><strong  style="font-size:130%"><%=arts.getAtitle()%></strong></a>
</p>
<p id="p03"><%=str%></p>
<p id="p04"><%=arts.getAtime()%>
<hr width=100% size="5">
<%
        i++;
    }
...
<ul >
	...
	<c:if test="<%=pageIndex%>>1 }" >
				<a href="all_essay2.jsp?pageIndex=1">首页&nbsp;</a>
				<a href="all_essay2.jsp?pageIndex=<%=pageIndex-1%>">上一页&nbsp;</a>
	</c:if>
	<c:if test="<%=pageIndex%> < <%=totalPage%> }" >
				<a href="all_essay2.jsp?pageIndex=<%=pageIndex+1%>">下一页&nbsp;</a>
				<a href="all_essay2.jsp?pageIndex=<%=totalPage%>">最后一页</a><br>
	</c:if>

 <form href="all_essay.jsp">
 <span >跳转至&nbsp;
