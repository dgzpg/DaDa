...
<a href="essay_content.jsp?name=<%=arts.getAno()%>"><strong style="font-size:130%"><%=article.getAtitle()%></strong></a>
<a href="<%=request.getContextPath()%>/cn.rt.servlet/CommentServlet?action=del&ano=<%=ano%>&cno=<%=arts.getCno()%>"
						style="float: right" type="submit">删除</a>
...
<ul >
	...
	<c:if test="<%=pageIndex%>>1 }" >
				<a href="my_comment.jsp?pageIndex=1">首页&nbsp;</a>
				<a href="my_comment.jsp?pageIndex=<%=pageIndex-1%>">上一页&nbsp;</a>
	</c:if>
	<c:if test="<%=pageIndex%> < <%=totalPage%> }" >
				<a href="my_comment.jsp?pageIndex=<%=pageIndex+1%>">下一页&nbsp;</a>
				<a href="my_comment.jsp?pageIndex=<%=totalPage%>">最后一页</a>
	</c:if>


 <form href="my_comment.jsp">
 <span >跳转至
